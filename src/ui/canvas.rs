...
pub fn draw_canvas(state: &AppState, ctx: &egui::Context, actions: &mut Vec<Action>) {
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        // Skip canvas keyboard shortcuts when a text widget has focus
        let text_focused = ctx.wants_keyboard_input();

        if !text_focused && ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
        ...
        let mut nudge_x = 0.0;
        let mut nudge_y = 0.0;
        if !text_focused && (response.hovered() || response.has_focus()) {
        ...
        if let Some(mouse_pos) = mouse_pos {
            ...
            // Removed auto-reset to Select when clicking outside canvas
        ...
        let draw_point_target = |sp: Pos2, col: Color32, is_selected: bool, is_hovered: bool| {
            ...
            // Draw the colored circle interior
            painter.circle_filled(sp, radius, col);

            // Draw a white border, then a black border on the outside for contrast
        ...
        if !state.calib_pts.is_empty() {
            ...
            let draw_dashed_line_inf =
                |painter: &egui::Painter, p1: Pos2, dir: Vec2, stroke: Stroke| {
                    ...
                    // Project the center of the screen onto our line to know where to start drawing
                    let center = painter.clip_rect().center();
                    let to_center = center - p1;
                    let t_center = to_center.x * dir.x + to_center.y * dir.y;

                    // Start drawing 4000 pixels before the screen center projection
                    let t_start = t_center - 4000.0;
                    // Snap t to a multiple of dash_period so the dashes don't "slide" as you pan
        ...
        let is_alt_pressed = ctx.input(|i| i.modifiers.alt);
        let is_space_held = ctx.input(|i| i.key_down(egui::Key::Space));
        let effective_mode_cursor = if is_space_held {
            AppMode::Pan
        } else if is_alt_pressed
            && (state.mode == AppMode::Select || state.mode == AppMode::AddData)
        {
        ...
        if effective_mode_cursor == AppMode::Delete && response.hovered() {
            ctx.set_cursor_icon(egui::CursorIcon::Crosshair);
        } else if effective_mode_cursor == AppMode::Pan && response.hovered() {
            ctx.set_cursor_icon(egui::CursorIcon::Grab);
        } else if (effective_mode_cursor == AppMode::AddData
            || effective_mode_cursor == AppMode::AddCalib)
            && response.hovered()
        {
            ctx.set_cursor_icon(egui::CursorIcon::Crosshair);
